...
<div>
  <ul class="login-top">
    ...
    <li class="login-title">新增收货地址</li>
  </ul>
  <div id="user-info">
    ...
    <div class="user-btn">
      <input type="button" value="确定" @click="add()" @focus="focus='btn'">
    </div>    
...
<script>
import {addAddr} from '../../assets/js/config'
  export default {
    data() {
      return {
        focus:'',//当前焦点在哪
    ...
    methods:{
      ...
      add(){
        if(this.username==""){
            ...
            this.errorText = "添加成功";
            addAddr({username:this.username,address:this.address,tel:this.tel,isDefault:this.isDefault});
            const that = this;
            setTimeout(function (){
                //走注册方法之后 直接进login
                that.$router.back(-1);
